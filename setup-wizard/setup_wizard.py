...
def update_docker_compose_labels(domain, use_letsencrypt):
    ...
    # Determine ca subdomain
    ca_domain = f"ca.{domain}" if domain != 'localhost' else 'ca.localhost'
...
def create_env_file(config):
    ...
    # Determine domain prefix for services
...
DOMAIN={domain}
SCEP_SERVER_URL=https://{ca_domain}/scep
...
def setup_configuration():
    ...
    try:
        ...
        config = {
            ...
            'admin_password': 'admin',  # Default password, user should change after login
...
def deploy_application():
    ...
    try:
        if not os.path.exists('/app/output/setup_complete.flag'):
            return jsonify({'error': 'Setup not completed'}), 400
        
        # Update docker-compose.yml with new labels
        ...
        with open('/app/output/cleanup_instructions.json', 'w') as f:
            json.dump(cleanup_instructions, f, indent=2)
        
        # Signal that deployment should start
        with open('/app/output/deploy_ready.flag', 'w') as f:
            f.write('ready')
